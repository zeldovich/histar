...
static struct bf_ctx id_key_ctx;
uint64_t id_counter;
...
key_derive(void)
{
    ...
    uint64_t ctr = 0;
    ctr = key_derive_one(&init_ctx, &id_key_ctx, ctr);
...
id_alloc(void)
{
    uint64_t new_count = id_counter++;
    uint64_t new_crypt = bf61_encipher(&id_key_ctx, new_count);

    if (id_counter >= (UINT64(1) << 61))
	panic("This HiStar instance is over.");

    uint64_t h = id_encrypt ? new_crypt : new_count;
    if (h == 0)
	h = id_alloc();
