	      ...
	      struct btree_node *prevNode, int div)
{
	...
	btree_keycpy(btree_key(prevNode->keys, div, tree->s_key),
		     btree_key(rootNode->keys, (int) rootNode->keyCount,
			       tree->s_key), tree->s_key);
		  ...
		  struct btree_node *prevNode, int div)
{
	...
	temp2 = btree_value(node->children, 0, tree->s_value);
	btree_valcpy(temp1, temp2, tree->s_value);
		 ...
		 struct btree_node *prevNode, int div)
{
	...
	temp2 =
	    btree_value(node->children, node->keyCount - 1, tree->s_value);
	btree_valcpy(temp1, temp2, tree->s_value);
	    ...
	    struct btree_node *rootNode, struct btree_node *prevNode, int div)
{
	...
	for (j = 0; j < node->keyCount; j++, i++) {
	    ...
	    btree_keycpy(btree_key(rootNode->keys, i, tree->s_key),
			 btree_key(node->keys, j, tree->s_key), tree->s_key);
