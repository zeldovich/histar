...
static pthread_mutex_t s_pendingRequestsMutex;
static pthread_mutex_t s_writeMutex;
static pthread_mutex_t s_startupMutex;
static pthread_cond_t s_startupCond;

static pthread_mutex_t s_dispatchMutex;
static pthread_cond_t s_dispatchCond;
 ...
 */
extern "C" void
RIL_issueLocalRequest(int request, void *data, int len)
{
    RequestInfo *pRI;
    int ret;
...
static void debugCallback (int fd, short flags, void *param)
{
    ...
    switch (atoi(args[0])) {
        case 0:
            ...
            RIL_issueLocalRequest(RIL_REQUEST_RESET_RADIO, NULL, 0);
            break;
        case 1:
        ...
        case 3:
            ...
            qxdm_data[4] = 0;
            qxdm_data[4] = 8;
            RIL_issueLocalRequest(RIL_REQUEST_OEM_HOOK_RAW, qxdm_data, 
                              6 * sizeof(int));
            break;
        case 4:
        ...
        case 6:
            ...
            actData[0] = args[1];
            RIL_issueLocalRequest(RIL_REQUEST_SETUP_DEFAULT_PDP, &actData, 
                              sizeof(actData));
            break;
        case 7:
            LOGI("Debug port: Deactivate PDP");
            RIL_issueLocalRequest(RIL_REQUEST_DEACTIVATE_DEFAULT_PDP, &deactData, 
                              sizeof(deactData));
            break;
        case 8:
...
RIL_init(void)
{
	pthread_mutex_init(&s_pendingRequestsMutex, NULL);
	pthread_mutex_init(&s_writeMutex, NULL);
	pthread_mutex_init(&s_startupMutex, NULL);
	pthread_mutex_init(&s_dispatchMutex, NULL);
	pthread_cond_init(&s_startupCond, NULL);
	pthread_cond_init(&s_dispatchCond, NULL);
}
