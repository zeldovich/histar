...
int __android_log_print(int, const char *, const char *, ...) __attribute__((__format__ (__printf__, 3, 0)));;
int
__android_log_print(int level, const char *tag, const char *fmt, ...)
{
	va_list va;

	cprintf("android_log: level %d, tag '%s': ", level, tag);
	va_start(va, fmt);
	vcprintf(fmt, va);
	va_end(va);
	cprintf("\n");
...
android_get_control_socket(const char *name)
{
	...
	if (strcmp(name, "rild") == 0) {
		...
		cprintf("%s: unknown control socket name '%s'\n", __func__, name);
		return (-1);
	}

	int s = socket(AF_UNIX, SOCK_STREAM, 0);
	if (s < 0) {
		cprintf("%s: socket() failed\n", __func__);
		return (s);
	}
