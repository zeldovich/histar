...
smd_read(int fd, void *buf, size_t nbyte)
{
	...
	if (residlen) {
		...
		ret = smddgate_tty_read(0, (unsigned char *)buf, nbyte);
	}
...
qmi_open(const char *path, int flags)
{
	...
	if (strncmp(path, "/dev/qmi0", 9) == 0) {
		if (smddgate_qmi_open(0) < 0)
			return (-1);
		return (qmi0_fd);
	} else if (strncmp(path, "/dev/qmi1", 9) == 0) {
		if (smddgate_qmi_open(1) < 0)
			return (-1);
		return (qmi1_fd);
	} else if (strncmp(path, "/dev/qmi2", 9) == 0) {
		if (smddgate_qmi_open(2) < 0)
			return (-1);
		return (qmi2_fd);
	}
...
qmi_read(int fd, void *buf, size_t nbyte)
{
	...
	ssize_t ret = smddgate_qmi_read(n, (unsigned char *)buf, nbyte); 
