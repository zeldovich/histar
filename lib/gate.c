		       ...
		       struct gate_entry_stack_info *stackinfp)
{
    ...
    int r = sys_obj_get_label(return_gate, &gate_label);
    if (r < 0)
    ...
    struct ulabel return_clearance = { .ul_nent = 0, .ul_default = 2 };

    r = sys_gate_enter(return_gate,
		       &gate_label, &return_clearance,
...
gate_call_return(struct gate_return *gr, struct cobj_ref arg)
{
    ...
    if (gate_debug)
	cprintf("gate_call_return: after taint_cow\n");
...
gate_call(struct cobj_ref gate, struct cobj_ref *argp)
{
	...
	r = sys_obj_get_label(gate, &gate_label);
	if (r < 0) {
	...
	gate_args->arg = *argp;
	r = sys_gate_enter(gate, &gate_label, 0, 0, 0);
	if (r == 0)
